...
async function transcribirGroq(audioBuffer, mimetype, key) {

    let groqClient = null;

    if (!key) {
        console.error("[OIR.JS] ERROR: La Key de Groq no fue proporcionada para inicializar el módulo Oir.");
        throw new Error("La API Key de Groq no está configurada para el módulo Oir.");
    }
    groqClient = new OpenAI({
        apiKey: key,
        baseURL: "https://api.groq.com/openai/v1"
    });
    console.log("[OIR.JS] Módulo Oir inicializado con Groq.");


    if (!groqClient) {
        console.error("[OIR.JS] ERROR: El cliente Groq no está inicializado para transcribir.");
    ...
    if (!supportedMimeTypes.includes(cleanedMimeType)) {
        console.error(`[OIR.JS] ERROR: Tipo de archivo de audio no soportado por Groq después de la limpieza: ${cleanedMimeType}. Original: ${mimetype}.`);
        throw new Error(`Tipo de archivo de audio no soportado por Groq: ${cleanedMimeType}. Tipos soportados: ${supportedMimeTypes.join(', ')}`);
    }
...
module.exports = { transcribirGroq };